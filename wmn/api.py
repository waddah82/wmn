...
def get_past_order_list(search_term, status, limit=20):
    """WMN Recent Orders extension for ERPNext v16.

    ERPNext v16 identifies each result by `(doctype, name)`. WMN preserves that
    contract while adding Awaiting Cashier plus separate Unpaid/Overdue filters.
    """
    ...
    def filters_for(doctype):
        ...
        if doctype == "Sales Invoice":
            ...
            if status == "Return":
                ...
                filters["is_return"] = 0
                filters["status"] = "Paid"
            elif status in ("Partly Paid", "Unpaid", "Overdue"):
                filters["is_return"] = 0
    ...
    for doctype in ("POS Invoice", "Sales Invoice"):
        ...
        doctype_rows = frappe.db.get_list(
            doctype,
            filters=filters,
        ...
        for row in doctype_rows:
            if status == "Draft" and str(row.get("wmn_pos_stage") or "").strip() == "AWAITING_CASHIER":
                continue
