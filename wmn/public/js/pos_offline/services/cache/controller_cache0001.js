...
(function () {
    ...
    class WMNPOSControllerCache {
        ...
        async getInvoiceDoctype(defaultDoctype) {
            ...
            if (cint(profileAsSalesInvoice ?? settings.as_sales_invoice ?? 0) === 1) return "Sales Invoice";
        ...
        async makeReturnInvoiceOffline(sourceDoc) {
            ...
            // ERPNext return documents preserve invoice-level pricing state from the source.
            // Keep the same discount basis and reverse fixed discount/tax amounts before
            // recalculating locally so Offline produces the same business result as Online.
            frm.doc.apply_discount_on = sourceDoc.apply_discount_on || frm.doc.apply_discount_on || "Grand Total";
            frm.doc.additional_discount_percentage = flt(sourceDoc.additional_discount_percentage || 0);
            frm.doc.discount_amount = -flt(sourceDoc.discount_amount || 0);
            frm.doc.base_discount_amount = -flt(
                sourceDoc.base_discount_amount !== undefined
                    ? sourceDoc.base_discount_amount
                    : sourceDoc.discount_amount || 0
            );
            frm.doc.taxes_and_charges = sourceDoc.taxes_and_charges || "";
            frm.doc.tax_category = sourceDoc.tax_category || "";
            frm.doc.taxes = (sourceDoc.taxes || []).map((tax, idx) => {
            ...
            frm.doc.items = (sourceDoc.items || []).map((row, idx) => {
                ...
                if (frm.doc.doctype === "Sales Invoice") {
                    returnRow.sales_invoice_item = row.name;
                } else if (frm.doc.doctype === "POS Invoice") {
                    returnRow.pos_invoice_item = row.name;
                }
            ...
            if (sourcePaidAmount > 0.000001 && !hasMappedPayment) {
                ...
                if (fallbackMethod && fallbackMethod.mode_of_payment) {
                    frm.doc.payments.push({
                        doctype: "Sales Invoice Payment",
                        name: "OFFLINE-RETURN-PAY-" + Date.now() + "-fallback",
                        parent: frm.doc.name,
                        parenttype: frm.doc.doctype,
                        parentfield: "payments",
                        idx: frm.doc.payments.length + 1,
                        mode_of_payment: fallbackMethod.mode_of_payment,
                        type: fallbackMethod.type || "",
                        account: fallbackMethod.account || "",
                        default: fallbackMethod.default,
                        amount: -sourcePaidAmount,
