...
(function () {
    ...
    async function renderDocuments(dialog, config, searchValue) {
        ...
        $list.html(rows.map((row) => {
            const state = String(row.__wmn_sync_status || "clean");
            const stateLabel = state === "pending_create" ? __("Pending Create")
                : state === "pending_update" ? __("Pending Update")
                : state === "conflict" ? __("Conflict")
                : state === "error" ? __("Sync Error")
                : "";
            return `
            <button type="button" class="wmn-pos-doctype-row" data-name="${escapeHtml(row.name)}" data-sync-status="${escapeHtml(state)}" data-local-record="${row.__wmn_local_record ? "1" : "0"}">
                <div style="min-width:0;"><strong>${escapeHtml(rowTitle(row, config))}</strong>${rowListDetails(row, config) ? `<small style="display:block;margin-top:2px;color:var(--text-muted,#6c7680);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(rowListDetails(row, config))}</small>` : ""}</div>
                <span>${escapeHtml(row.name)}</span>
                <span>${escapeHtml(formatModified(row.modified))}${stateLabel ? ` <em class="wmn-pos-doctype-sync-state" data-state="${escapeHtml(state)}">${escapeHtml(stateLabel)}</em>` : ""}</span>
            </button>`;
        }).join(""));
    }
