/* WMN POS page Service Worker v16-port
   Changes:
...
const WMN_POS_SW_VERSION = "wmn-pos-page-20260910-v3";
const WMN_POS_CACHE = "wmn-pos-page-runtime-20260910-v3";
const WMN_POS_API_CACHE = "wmn-pos-page-api-20260910-v3";
...
function normalizedSearchFor(url, body) {
  ...
  if (p === "/api/method/frappe.desk.desk_page.getpage") {
    const name = url.searchParams.get("name") || getBodyParam(body, "name") || "wmn-pos";
...
function fallbackApi(url, request) {
  ...
  if (p === "/api/method/wmn.wmn.page.wmn_pos.wmn_pos.get_pos_offline_data") {
    return jsonResponse({
      message: {
        offline: true,
        items: [],
  ...
  return jsonResponse({
    message: "Offline request blocked by WMN POS page service worker",
    _wmn_offline: true
  }, 503);
}
  ...
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(
        keys
          .filter((key) => key.startsWith("wmn-pos-page-") && ![WMN_POS_CACHE, WMN_POS_API_CACHE].includes(key))
...
self.addEventListener("fetch", (event) => {
  ...
  if (request.method === "GET") {
    ...
    if (isAsset(url) || url.pathname === "/wmn-pos-manifest.json" || url.pathname === "/wmn-pos-manifest.webmanifest") {
      event.respondWith(
