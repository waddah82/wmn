frappe.query_reports["Acccount Statement"] = {
    ...
    onload: function (report) {
        report.page.set_primary_action(__('Print'), function () {
            ...
            let company_name = frappe.defaults.get_user_default("Company");
...
`);
        ...
        <tr style="background-color: ${idx % 2 === 0 ? '#F8F8F8' : '#FFFFFF'};">
            ...
            <td style="text-align:right;">
                <span class="amount">${parseFloat(row.running_balance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                <span class="currency">${currency}</span>
            </td>
        </tr>
    `);
